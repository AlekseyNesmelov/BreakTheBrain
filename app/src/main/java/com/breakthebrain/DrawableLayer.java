...
public class DrawableLayer {
    ...
    /**
     * Gets draggable object which contains point.
     * @param x point x.
     * @param y point y.
     * @return draggable object which contains point.
     */
    public DrawableObject getDraggableObjectInside(final float x, final float y) {
        for (final DrawableObject object : mObjects) {
            if (object.isInside(x, y) && object.isDraggable()) {
                return object;
            }
        }
        return null;
